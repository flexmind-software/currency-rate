...
use FlexMindSoftware\CurrencyRate\Drivers\IsraelDriver;

class IsraelDriverTest extends TestCase
{
    ...
    public function it_parses_israel_response()
    {
        $driver = (new class () extends IsraelDriver {
            protected function fetch(string $url, array $query = []): ?string
            {
                if (str_contains($url, 'currency.xml')) {
                    return file_get_contents(__DIR__.'/Fixtures/israel_currencies.xml');
                }

                return file_get_contents(__DIR__.'/Fixtures/israel_rates.html');
            }
        ...
        $this->assertEquals(3.5, $usd->rate);
